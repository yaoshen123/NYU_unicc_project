...
title = """<h1 align="center">media analysis Demo</h1>"""
description = 'Welcome to Our media analysis Chatbot Demo!'


introduction = '''
For Abilities Involging Multimodal media analysis tool:
1. Reason: Click **Send** to generate a multimodal reason description.
2. Visual: Click **Send** to generate a visual description.
3. Audio: Click **Send** to generate an audio description.
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            chatbot = gr.Chatbot(label='media analysis')

            dataset = gr.Dataset(
                components=[gr.Textbox(visible=False)],
                samples=[['No Tag'], ['Reason'], ['Visual'], ['Audio']],
                type="index",
    ...
    with gr.Row():
        with gr.Column():
            gr.Examples(examples=[
                ["examples/paiwai.mp4", "[reason] In the video, the speaker says things like These immigrants come here and take our jobs and They don't belong here and should go back to their country. Does this indicate any xenophobic, racist, or anti-immigrant statements or implications?", upload_flag, replace_flag, img_list],
                ["examples/Anti-immigration_clip.mp4", "[visual] What actions did the police take toward immigrants in the video, and do these actions—when combined with the political commentary—create a narrative that could be perceived as racist, anti-immigrant, or xenophobic?", upload_flag, replace_flag, img_list],
                ["examples/Audio_test.mp4", "[audio] Please summarize the content of the audio and determine whether it contains any xenophobic, racist, or anti-immigrant statements or implications.", upload_flag, replace_flag, img_list],
        ...
        with gr.Column():
            gr.Examples(examples=[
                ...
                ["examples/AGGHufY.mp4", "Please judge whether the tweet contains any xenophobic, racist, or anti-immigrant statements or implications. Search Hillary not no better going to take your your freedom and bring in more refugees to destroy this country . Neither may not be good but I want to save my country . And not any one take it from us Muslim have no right to bring their laws here and say take God out of schools the ten commendments away so screw it we the people have the right to save America no more illegals no more refugees no more free tax for non Americans every body pays taxes and no more tax credits for non Americans . Muse come a citizen to work here . Or we all be walking the trail of tears again only worse 2 hours ago Like Reply.", upload_flag, replace_flag, img_list],
                # not
                ["examples/Chinese_sample.mp4", "Please judge whether the tweet contains any xenophobic, racist, or anti-immigrant statements or implications. 他们这样也不错。去年和英国朋友讨论脱欧及非法移民问题，我说英国人投脱欧的傻X，大批难民死在英吉利海峡不人道，把他说怒了，说怎么不让他们都去中国？我说谢谢你们的宣传啊，BBC,CNN之类的，把中国说成地狱似的，我们欢迎难民，难民都看不上中国，就喜欢你们的民主自由他气的…把我爽的…", upload_flag, replace_flag, img_list],
                ["examples/Arabic_sample.mp4", "Please judge whether the tweet contains any xenophobic, racist, or anti-immigrant statements or implications. ليعلم الحوثي وغيره اننا سنعود الى صنعاء ليس عودة لاجئ بل سنعود وقد ازلنا الحوثي واخرجناه من البلاد واستعدنا الجمهورية كلمة قائد مقاومة صنعاء الشيخ منصور الحنق أثناء تشييع جثمان نجله عبدالله رحمه الله", upload_flag, replace_flag, img_list],
                ["examples/sample.mp4", "Please review the following news excerpt for factual accuracy and appropriate terminology, especially with regard to migrants, refugees, and displaced populations. Use verified fact-checking sources and terminology databases (e.g., UNHCR, IOM, AP Stylebook) to identify and correct any outdated, misleading, or inappropriate terms. Highlight any factual inaccuracies and suggest clear corrections or clarifications grounded in credible, up-to-date sources. Excerpt for Review:Illegal aliens have been flooding the border in record numbers, overwhelming the system and putting American citizens at risk.", upload_flag, replace_flag, img_list],
                ["examples/sample.mp4", "Identify and categorize harmful or misleading narratives in the following media excerpt related to migration and displacement, highlighting topics that contribute to negative public perception. As Theodore Roosevelt stated, in order to immigrate and live in another country successfully one must fully integrate. Then there will be no opposition against it.", upload_flag, replace_flag, img_list],
