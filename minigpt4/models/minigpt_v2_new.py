...
class MiniGPTv2(MiniGPTBase):
    ...
    def __init__(
        ...
        self.feats_llama_proj_DSGG = nn.Linear(
            256, self.llama_model.config.hidden_size
        )

        self.feats_llama_proj_motion = nn.Linear(
            1024, self.llama_model.config.hidden_size
        )
    ...
    def encode_img(self, image, video_features):
        ...
        with self.maybe_autocast():
            # # VideoMAE_only
            # video_features = video_features.to(device)   # [1, 4, 1024]
            ...
            # # output_DSGG = self.feats_llama_proj_DSGG(video_features_split[-1].squeeze(1)[:, :256])     
            # video_feats = torch.stack([output2,output3], dim=1)
            
            
            # inputs_llama = video_feats
            ...
            # output3 = self.feats_llama_proj3(video_features_split[2].squeeze(1))
            # # output_DSGG = self.feats_llama_proj_DSGG(video_features_split[-1].squeeze(1)[:, :256])     
            # video_feats = torch.stack([output1,output3], dim=1)
            ...
            # video_features = video_features.to(device)   # [1, 4, 1024]
            # video_features_split = torch.split(video_features, 1, dim=1)
            # output1 = self.feats_llama_proj1(video_features_split[2].squeeze(1))
            # video_feats = output1.unsqueeze(1) 
            ...
            output1 = self.feats_llama_proj1(video_features_split[2].squeeze(1))
            # output_DSGG = self.feats_llama_proj_DSGG(video_features_split[-1].squeeze(1)[:, :256])     
            # video_feats = torch.stack([output1,output_DSGG], dim=1)
            output_motion = self.feats_llama_proj_motion(video_features_split[-1].squeeze(1))
            video_feats = torch.stack([output1, output_motion], dim=1)
            ...
            # output2 = self.feats_llama_proj2(video_features_split[1].squeeze(1))
            # output3 = self.feats_llama_proj3(video_features_split[2].squeeze(1))
            # output_DSGG = self.feats_llama_proj_DSGG(video_features_split[3].squeeze(1)[:, :256])     
            # video_feats = torch.stack([output1, output2, output3,output_DSGG], dim=1)
            # # output_motion = self.feats_llama_proj_motion(video_features_split[3].squeeze(1))
            # # video_feats = torch.stack([output1, output2, output3, output_motion], dim=1)
